...
def validar_entrada(valor_veiculo, entrada):
    minimo = valor_veiculo * 0.10
    if entrada < minimo:
        raise ValueError("A entrada mínima é de 10% do valor do veículo.")
    return True


def validar_prazo(prazo, tipo):
    if tipo == 'financiamento' and prazo > 120:
        raise ValueError("Prazo máximo para financiamento é 120 meses.")
    if tipo == 'consorcio' and prazo > 180:
        raise ValueError("Prazo máximo para consórcio é 180 meses.")
    return True
